...
const Home = () => {
  ...
  return (
    <>
      ...
      {/* <h1>{cv.data.name.raw}</h1>
      <p>{cv.data.phoneNumbers[0]}</p>
      <p>{cv.data.websites[0]}</p>
      <p>{cv.data.emails[0]}</p>
      <p>
        <span>{cv.data.location.city}, </span>
        <span>{cv.data.location.state}, </span>
        <span>{cv.data.location.country}. </span>
      </p>
      <h5>{cv.data.objective}</h5> */}
    </>
  )
}
